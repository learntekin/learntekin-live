...
export default class Careers extends Component {
  ...
  handleSubmit = (event) => {
    ...
    // Collect form data
    const formData = {
      title: event.target.options.value, // Assuming options is the title (Ms./Mr./Mrs.)
      firstName: event.target.Fname.value,
      lastName: event.target.Lname.value,
      email: event.target.email.value,
      phoneNumber: event.target.phoneN.value,
      role: event.target.roleN.value,
      referral: event.target.referN.value,
    ...
    // Push data to the "applications" node in the Firebase Realtime Database
    database
    .ref("careers")
    .push(formData)
    .then(() => {
      ...
      this.setState({
        successMessage: 'Data successfully sent to Firebase!',
        error: null,
      });
    })
    .catch((error) => {
      ...
      this.setState({
        successMessage: null,
        error: `Error sending data to Firebase: ${error.message}`,
      });
    });
  };
  componentDidMount() {
              ...
              <div class="info-box1 ">
                <div class="col-lg-12 mt-6">
                  ...
                  >
                    ...
                    <div class="my-3">
                      <div class="loading">  {error && <p>{error}</p>}</div>
