...
import firebase from "../../firebase";  // Make sure your firebase config is correct
...
const Login = () => {
  ...
  useEffect(() => {
    // This checks if the user is already logged in when the component mounts
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        // User is logged in, update Redux state and navigate to home page
        dispatch(loginSuccess(user));
        history.push("/");  // Redirect to home page if logged in
      }
    });
  }, [dispatch, history]);
  ...
  const handleLogin = () => {
    firebase
      ...
      .catch((error) => {
        ...
        toast.error(`Login failed: ${errorMessage}`); // Show error toast
      });
  };
